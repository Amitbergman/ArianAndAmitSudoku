...
SudokuBoard* gurobi(SudokuBoard* board)
{
	GRBenv   *env   = NULL;
	GRBmodel *model = NULL;
	int i,j,k,igrid, jgrid,count,m,n,N;
	int       error = 0;
	double*    sol;
	int*       ind;
	double*    val;
	...
	printf("m=%d, n=%d\n",solBoard->m,solBoard->n);

	/* Create environment - log file is mip1.log */
	error = GRBloadenv(&env, "mip1.log");
	if (error) {
		printf("ERROR %d GRBloadenv(): %s\n", error, GRBgeterrormsg(env));
		freeAll(env, model, sol,ind, val, lb, vtype);
		return NULL;
	}
  ...
  if (error) {
	...
	for (i = 0; i < N; i++) {
		for (j = 0; j < N; j++) {
			...
			if (error) {
				printf("ERROR 1 %d GRBaddconstr(): %s\n", error, GRBgeterrormsg(env));
				freeAll(env, model, sol,ind, val, lb, vtype);
				return NULL;
			}
		}
	...
	for (k = 0; k < N; k++) {
		for (j = 0; j < N; j++) {
			...
			error = GRBaddconstr(model, N, ind, val, GRB_EQUAL, 1.0, NULL);
			if (error) {
				printf("ERROR 2 %d GRBaddconstr(): %s\n", error, GRBgeterrormsg(env));
				freeAll(env, model, sol,ind, val, lb, vtype);
				return NULL;
			}
		}
	...
	for (k = 0; k < N; k++) {
		for (i = 0; i < N; i++) {
			...
			error = GRBaddconstr(model, N, ind, val, GRB_EQUAL, 1.0, NULL);
			if (error) {
				printf("ERROR 3 %d GRBaddconstr(): %s\n", error, GRBgeterrormsg(env));
				freeAll(env, model, sol,ind, val, lb, vtype);
				return NULL;
			}
		}
	...
	for (k = 0; k < N; k++) {
		for (igrid = 0; igrid < n; igrid++) {
			for (jgrid = 0; jgrid < m; jgrid++) {
				...
				error = GRBaddconstr(model, N, ind, val, GRB_EQUAL, 1.0, NULL);
				if (error) {
					printf("ERROR 4 %d GRBaddconstr(): %s\n", error, GRBgeterrormsg(env));
					freeAll(env, model, sol,ind, val, lb, vtype);
					return NULL;
				}
			}
		...
		for (i=0;i<N;i++){
			for (j=0;j<N;j++){
				for (k=0;k<N;k++){
					...
					/* printf("i=%d j=%d k=%d sol=%f",i,j,k,sol[i*N*N+j*N+k]); */

					if (sol[i*N*N+j*N+k]==((double)1)){
						solBoard->board[i][j].content=k+1;
						printf("i=%d j=%d k=%d sol=%f",i,j,k+1,sol[i*N*N+j*N+k]);
